...
const EditWorkerModalWindow = ({ worker, status }) => {
  ...
  useEffect(() => {
    ...
    if (!toSave && (photo || worker.photo) && validFullName && validSpecialty && validExperience && validCertificates) {
      setToSave(true);
    }
    else if (toSave && ((!photo && !worker.photo) || !validFullName || !validSpecialty || !validExperience || !validCertificates)) {
      setToSave(false);
    }
  ...
  async function addNewWorker(event) {
    ...
    if (photo) {
      ...
      formData.append('photoPath', worker.photo);
    }
    ...
    $api.put(`/workers/${worker.id}`, formData)
      .then(response => {
      ...
      <div className={styles.window}>
        ...
        <form className={styles.form} onSubmit={addNewWorker}>
          ...
          <div className={styles.about}>
            <label className={styles.loadImage} style={{backgroundImage: `url(${photo ? URL.createObjectURL(photo) : worker.photo})`}}>
              {(!photo && !worker.photo) && 
...
export default EditWorkerModalWindow;
